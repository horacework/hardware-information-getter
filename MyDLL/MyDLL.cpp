...
// 验证本机主板序列号与CPU序列号是否与固定的一致
BOOL Verify_BoardIDAndCpuID(){
	T_DEVICE_PROPERTY mainboardID;
	T_DEVICE_PROPERTY cpuID;
	WMI_DeviceQuery(2, &mainboardID, 1);
	WMI_DeviceQuery(3, &cpuID, 1);
	return (_tcscmp(mainboardID.szProperty, D_mainboardID)==0) && (_tcscmp(cpuID.szProperty, D_cpuID)==0);
}